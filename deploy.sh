...
# Stack Name variable
STACK_BUCKET_NAME="nu0087002eia-aid-dev-bucket-test-stack"
STACK_MAIN_NAME="nu0087002eia-aid-dev-main-test-stack"
# Update switch
UPDATE_SWITCH=${1:-"no"}
...
if [[ $UPDATE_SWITCH == "no" ]]; then
    ...
    if [[ $UPDATE_ARTIFACT == "si" ]]; then
        ...
        aws s3 cp --recursive ./carta_laboral s3://nu0087002eia-aid-mlops-artifacts-bucket
